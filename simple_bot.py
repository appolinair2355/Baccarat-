...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match_numero:
        ...
        # Check for progress indicators that require waiting for confirmation
        ...
        # If message has progress indicators but no confirmation, don't process yet
        if has_progress and not has_confirmation:
            logger.info(f"Message #{numero} has progress indicators, waiting for confirmation")
            return
        
        # Mark as processed only after confirmation or if no progress indicators
